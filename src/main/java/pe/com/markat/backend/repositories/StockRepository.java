...
public interface StockRepository extends JpaRepository<Stock, Long> {
    @Query("select s from Stock s where s.store.id=?1 order by s.noUnits asc")
    List<Stock> findAllStocksByStoreIdJPA(Long id);

    @Query("SELECT s FROM Stock s WHERE s.store.id =?1 AND s.noUnits < 20 order by s.noUnits desc")
